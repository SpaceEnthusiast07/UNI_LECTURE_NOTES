...
To keep the processor running smoothly, several techniques are used:
 - **Forwarding (Bypassing)**
	 ...
	 - I.e. its like me passing a piece of paper to a friend directly without putting it down on the table 🤣😁.
	 - If the forwarding hardware detects that the previous ALU operation has written to the register that the current ALU operation uses, it will re-read from this register so it uses the updated value, not the old version.
 - **Pipeline Stalls (Bubbles)**
	 - If the hazard cannot be solved by forwarding, the processor must "*stall*".
	 - This means inserting a "NOP" (No Operation) or "bubble", *pausing all newer instructions until the hazard is cleared*.
