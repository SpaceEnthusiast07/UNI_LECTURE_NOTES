...
##### Control Flow
 ...
 - ***Asynchronous Execution***
	 ...
	 - ***Concurrency*** is when two or more jobs are scheduled to run at the same time, to avoid this, jobs are given *priorities*. Jobs with higher priorities execute first


##### Scheduling Jobs
 - Sometimes you want tasks to happen at specific times or in a particular order
 - You schedule jobs to happen later using: 
	 - `.enter(<delay>, <priority>, <function>, <args>)`
		 - `delay` is relative to when `.enter()` is executed
	 - `.enterabs(<time>, <priority>, <function>, <args>`
		 - `time` can be a specific time (e.g. `12:30:58pm`) or relative to when `.run()` is called (using `time.time() + 10`)
 - Use `.run()` to execute all jobs until the queue is empty
 - Use `.cancel()` to remove a job from the queue
...
# Define function used in jobs
def print_job_name(name):
    print(name)

# Add some jobs to the queue
j1 = s.enterabs(time.time() + 3, 1, print_job_name, ("Job 1",))
j2 = s.enterabs(time.time() + 1, 2, print_job_name, ("Job 2",))
j3 = s.enterabs(time.time() + 1, 1, print_job_name, ("Job 3",))
j4 = s.enterabs(time.time() + 5, print_job_name, ("Job 4",))

# Remove job 4 from queue
s.cancel(j4)

# Start to run each job in the queue
s.run()
...
```
 ...
 - `if __name__ == "__main__":`
	 - Means: "Only run this part of the program if this file is being run directly, not if it's being imported as a module"
	 - Why?: It prevents Flask from auto running if you import this file into another program
 - `app.run()`:
